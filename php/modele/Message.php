...
class Message {
    ...
    static function getLastUsersMessage($conn, $recepteur){
        $stmt = $conn->prepare("SELECT m1.id, m1.emetteur, m1.texte, u.pseudo, u.id AS idUser, m1.date, m1.has_been_read
                                FROM utilisateur u ,message m1 LEFT JOIN message m2
                                 ON (m1.emetteur = m2.emetteur AND m1.id < m2.id)
                                WHERE m2.id IS NULL AND m1.recepteur = ? AND u.id = m1.emetteur ORDER BY date DESC");
        if($stmt->execute(array($recepteur))){
            return new Feedback($stmt->fetchAll(PDO::FETCH_ASSOC), true, "");
        }else{
            return new Feedback(NULL, false, "");
        }
